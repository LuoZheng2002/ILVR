...
- Repository root: top-level entries you will use most
  ...
  - `eval.sh` / `eval.py` / `src/evaluate.py` — evaluation drivers (two variants provided). `eval.sh` wraps either evaluation script for batch evaluation.
...
- `src/` — main implementation
  ...
  - `src/task_preprocess.py` — dataset preprocessing / task-specific helpers.
  ...
  - `src/training_utils.py` — collection of token / sequence utilities and dataset helpers used across training and eval.
  ...
  - `src/evaluate.py` — evaluation script referencing the same preprocessing & decoding conventions as training.
...
- Data expectations
  ...
  - Example dataset root constants exist in code (`TRAIN_IMAGE_ROOT` in `src/task_preprocess.py` and `BASE_DATASET_DIR` in `src/evaluate.py`) — update these or pass correct paths via the wrapper scripts.
...
- How to run
  ...
  4) Alternatively call `python src/evaluate.py --model_dir <dir> --test_data_path <file> --task_name <task>` directly (see script for arg details).
...
- Key implementation notes to be aware of
  ...
  - The training loop expects that user vs assistant images are separated (see `collate_fn_stage1` and `remove_user_images` / `remove_assistant_images` helpers in `src/training_utils.py`).
  ...
  - The repo includes two evaluation variants: `eval.py` (more recent, includes timing and summary) and `src/evaluate.py` (full-featured). Either can be used; `eval.sh` expects you to choose which script to call via `EVAL_SCRIPT_PATH`.

- Useful file references
  - Training entry: `src/main.py`
  - Trainer implementation: `src/trainer.py` (class `CustomTrainerStage1`)
  - Preprocessing/task config: `src/task_preprocess.py`
  - Utilities and label/mask logic: `src/training_utils.py`
  - Evaluation: `src/evaluate.py`, `eval.py`
